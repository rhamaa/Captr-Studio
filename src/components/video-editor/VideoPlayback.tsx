...
import { applyClipTransition, getActiveClipTransition } from "./videoPlayback/clipTransition";
...
const VideoPlayback = forwardRef<VideoPlaybackRef, VideoPlaybackProps>(
	(
		...
		const applyWebcamBubbleLayoutRef = useRef<((zoomScale: number) => void) | null>(null);
		const trimRegionsRef = useRef<TrimRegion[]>([]);
		const clipRegionsRef = useRef<ClipRegion[]>(clipRegions);
		const speedRegionsRef = useRef<SpeedRegion[]>([]);
		const transitionOverlayGraphicsRef = useRef<Graphics | null>(null);
		...
		const applyWebcamBubbleLayout = useCallback(
			(zoomScale: number) => {
				...
				const screenContainer = containerRef.current;
		...
		useEffect(() => {
			applyWebcamBubbleLayoutRef.current = applyWebcamBubbleLayout;
		...
		const layoutVideoContent = useCallback(() => {
			...
			if (result) {
				...
				applyWebcamBubbleLayoutRef.current?.(animationStateRef.current.appliedScale || 1);
			}
		}, [
		...
		}, [pixiReady, videoReady, cropRegion]);
		...
		const syncWebcamMedia = useCallback(() => {
			...
			const webcamSyncedRate = getMediaSyncPlaybackRate({
				basePlaybackRate: targetPlaybackRate,
				currentTime: webcamVideo.currentTime,
				targetTime: mediaTargetTime,
				toleranceSeconds: 0.012,
				correctionWindowSeconds: 1.0,
				maxAdjustment: 0.06,
			});
			if (Math.abs(webcamVideo.playbackRate - webcamSyncedRate) > 0.001) {
				webcamVideo.playbackRate = webcamSyncedRate;
			}
		...
		useEffect(() => {
			...
			for (const region of audioRegions) {
				const audioEl = audioElementsRef.current.get(region.id);
				if (!audioEl) continue;

				const isInRange = timelineTimeMs >= region.startMs && timelineTimeMs < region.endMs;
				...
				// Calculate target time within the audio file
				const mediaTargetTime = Math.max(0, (timelineTimeMs - region.startMs) / 1000);
				const baseVolume = Math.max(0, Math.min(1, region.volume ?? 1));

				const duckingMultiplier =
					audioDuckingSettings?.enabled &&
					region.ducking !== false &&
					speechIntervals.length > 0
						? computeDuckingGain(timelineTimeMs, speechIntervals, audioDuckingSettings)
						: 1.0;

				const targetVolume = Math.max(0, Math.min(1, baseVolume * duckingMultiplier));
				...
				// Check sync drift
				const driftThreshold = isPlaying ? 0.3 : 0.02;
				if (Math.abs(audioEl.currentTime - mediaTargetTime) > driftThreshold) {
					try {
						audioEl.currentTime = mediaTargetTime;
					} catch {
						// ignore seek drift error
					}
				}
		...
		}, [currentTime, isPlaying, audioRegions, audioDuckingSettings, speechIntervals]);
		...
		useEffect(() => {
			...
			const transitionOverlayGraphics = new Graphics();
			transitionOverlayGraphics.visible = false;
			transitionOverlayGraphicsRef.current = transitionOverlayGraphics;
			videoContainer.addChild(videoSprite);
			videoContainer.addChild(transitionOverlayGraphics);
			...
			return () => {
				...
				if (transitionOverlayGraphicsRef.current) {
					videoContainer.removeChild(transitionOverlayGraphicsRef.current);
					transitionOverlayGraphicsRef.current.destroy();
					transitionOverlayGraphicsRef.current = null;
				}
				if (maskGraphics) {
		...
		useEffect(() => {
			...
			const ticker = () => {
				...
				applyClipTransition({
					...
					videoContainer,
					overlayGraphics: transitionOverlayGraphicsRef.current,
					bounds: baseMaskRef.current,
				});
		...
		return (
			...
			>
				...
				<SocialSafeZoneOverlay aspectRatio={aspectRatio} visible={showSocialSafeZone} />
				...
				{audioRegions?.map((region) => (
					<audio
						key={region.id}
						ref={(el) => {
							if (el) audioElementsRef.current.set(region.id, el);
							else audioElementsRef.current.delete(region.id);
						}}
						src={toFileUrl(region.audioPath)}
						preload="auto"
						style={{ display: "none" }}
					/>
				))}
