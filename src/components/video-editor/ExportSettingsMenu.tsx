...
} from "@/utils/exportPresetUtils";
import type { GpuEncoderInfo } from "./useNvidiaCudaExportOptIn";
...
interface ExportSettingsMenuProps {
	...
	gpuEncoders?: GpuEncoderInfo | null;
	...
	onSelectQuickPreset?: (preset: ExportQuickPreset) => void;
	onExport?: () => void;
	className?: string;
}
		...
		>
			<div className="mb-2 flex items-center justify-between">
				<span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
					{tSettings("export.quickProfiles", "Quick Profiles")}
				</span>
			</div>

			<div className="mb-3.5 grid grid-cols-2 gap-1.5">
				{EXPORT_QUICK_PRESETS.map((preset) => {
					const isActive = isMatchingQuickPreset(preset, {
						format: exportFormat,
					...
					const Icon =
						preset.iconName === "crown"
							? Crown
							: preset.iconName === "globe"
								? Globe
								: preset.iconName === "lightning"
									? Lightning
									: Image;
					...
					return (
						...
						>
							...
							<span className="mt-1 line-clamp-1 text-[10px] text-muted-foreground/80">
								{preset.description}
							</span>
						</button>
					);
			...
			<div className="mb-2 flex items-center justify-between">
				<span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
					{tSettings("export.customSettings", "Custom Settings")}
				</span>
			</div>
					...
					{!isLegacyModel && gpuEncoders ? (
						<div
							className={cn(
								...
								gpuEncoders.isHardwareAccelerated
								...
								<div className="min-w-0">
									<div className="flex items-center gap-1.5">
										<span className="text-[11px] font-semibold text-foreground truncate">
											{gpuEncoders.primaryEncoderLabel}
										</span>
										{gpuEncoders.isHardwareAccelerated ? (
											<span className="rounded bg-emerald-500/15 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-emerald-600 dark:text-emerald-400">
												5x GPU
											</span>
										) : null}
									</div>
									<p className="truncate text-[10px] text-muted-foreground/75">
										{gpuEncoders.isHardwareAccelerated
											? "Hardware accelerated encoder active"
											: "Standard software encoding"}
									</p>
								</div>
							</div>
							<span className="shrink-0 font-mono text-[10px] text-muted-foreground/70">
								{gpuEncoders.primaryEncoder.replace("h264_", "")}
							</span>
						</div>
					) : null}
