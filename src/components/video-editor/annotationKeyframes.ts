...
export function normalizePropertyKeyframes(value: unknown): PropertyKeyframe[] | undefined {
	...
	for (const raw of value) {
		...
		frames.push({
			bezier:
				Array.isArray(raw.bezier) &&
				raw.bezier.length === 4 &&
				raw.bezier.every(Number.isFinite)
					? [
							Math.max(0, Math.min(1, raw.bezier[0])),
							raw.bezier[1],
							Math.max(0, Math.min(1, raw.bezier[2])),
							raw.bezier[3],
						]
					: undefined,
			id: raw.id,
...
export function getAnnotationLocalTime(
	...
	return (
		(annotation.keyframeTimeOffsetMs ?? 0) +
		Math.max(
			0,
