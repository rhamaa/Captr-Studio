...
export const FONT_FAMILY_VALUES = [
	...
	{ value: "'Inter', sans-serif", labelKey: "Google: Inter" },
	{ value: "'Roboto', sans-serif", labelKey: "Google: Roboto" },
	{ value: "'Playfair Display', serif", labelKey: "Google: Playfair Display" },
	{ value: "'Oswald', sans-serif", labelKey: "Google: Oswald" },
	{ value: "'Montserrat', sans-serif", labelKey: "Google: Montserrat" },
];
			...
			<div className="flex-1 min-h-0 p-4 overflow-y-auto custom-scrollbar">
								...
								<div className="pl-2 border-l-2 border-[#2563EB]/20 space-y-4 ml-1">
									<div>
										...
										<Slider
											...
											onValueChange={([value]) => onStyleChange({ dropShadowBlur: value })}
											min={0}
											max={20}
											step={1}
											className="w-full"
									...
									<div>
										...
										<Slider
											...
											onValueChange={([value]) => onStyleChange({ dropShadowOffsetX: value })}
											min={-20}
											max={20}
											step={1}
											className="w-full"
									...
									<div>
										...
										<Slider
											...
											onValueChange={([value]) => onStyleChange({ dropShadowOffsetY: value })}
											min={-20}
											max={20}
											step={1}
											className="w-full"
											...
											<PopoverTrigger asChild>
												...
												>
													<div
														...
														style={{ backgroundColor: annotation.style.dropShadowColor || "rgba(0,0,0,0.5)" }}
													/>
													<span className="text-[10px] text-muted-foreground truncate flex-1 text-left">
														{annotation.style.dropShadowColor || "rgba(0,0,0,0.5)"}
													</span>
												</Button>
											</PopoverTrigger>
											<PopoverContent className="w-[260px] p-3 bg-editor-surface-alt border border-foreground/10 rounded-xl shadow-xl">
												<Block
													color={annotation.style.dropShadowColor || "rgba(0,0,0,0.5)"}
													colors={colorPalette}
													onChange={(color) => {
														onStyleChange({ dropShadowColor: color.hex });
													}}
				...
				{(annotation.type === 'image' || annotation.type === 'gif' || annotation.type === 'text') && (
					<div className="mt-4 space-y-4 pt-4 border-t border-foreground/10">
						<div className="grid grid-cols-2 gap-2">
							<div>
								...
								<Select
									...
									onValueChange={(value) => onAnimationChange?.({ animationIn: value as "none" | "fade" | "slide-up" })}
								>
									<SelectTrigger className="w-full bg-foreground/5 border-foreground/10 text-foreground h-9 text-xs">
							...
							<div>
								...
								<Select
									...
									onValueChange={(value) => onAnimationChange?.({ animationOut: value as "none" | "fade" })}
								>
									<SelectTrigger className="w-full bg-foreground/5 border-foreground/10 text-foreground h-9 text-xs">
										<SelectValue placeholder="None" />
									</SelectTrigger>
						...
						{((annotation.animationIn && annotation.animationIn !== "none") || (annotation.animationOut && annotation.animationOut !== "none")) && (
							<div>
								<label className="text-[10px] font-medium text-muted-foreground mb-2 block">
								...
								<Slider
									...
									onValueChange={([value]) => onAnimationChange?.({ animationDurationMs: value })}
									min={100}
									max={2000}
									step={50}
									className="w-full"
