...
export function interpolateCursorPosition(
	...
	// Use Catmull-Rom cubic spline across adjacent samples for cinematic, organic curves
	const p0 = lo > 0 ? samples[lo - 1] : a;
	const p1 = a;
	const p2 = b;
	...
	if (hasLargeGap) {
		return {
			...
			cy: Math.max(0, Math.min(1, a.cy + (b.cy - a.cy) * t)),
		};
	}
	...
	return {
		cx: catmullRom(p0.cx, p1.cx, p2.cx, p3.cx),
		cy: catmullRom(p0.cy, p1.cy, p2.cy, p3.cy),
	};
}
...
export class PixiCursorOverlay {
	...
	update(
		...
		if (clickProgress > 0) {
			const t = 1 - clickProgress;
			const easeOut = 1 - Math.pow(1 - t, 3);
			const viewportScale = getCursorViewportScale(viewport);
			const maxRadius = 46 * viewportScale;
			const minRadius = 6 * viewportScale;
			const radius = minRadius + easeOut * (maxRadius - minRadius);
			const ringAlpha = Math.pow(clickProgress, 1.3) * 0.85;
			const strokeWidth = Math.max(1.5, (3 - easeOut * 1.5) * viewportScale);

			this.clickRingGraphics.circle(px, py, radius * 0.82);
...
export function drawCursorOnCanvas(
	...
	if (clickProgress > 0) {
		...
		ctx.fillStyle = `rgba(59, 130, 246, ${ringAlpha * 0.22})`;
		ctx.fill();
