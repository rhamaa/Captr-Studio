...
export interface CursorFollowCameraState {
	...
	frozenFocusX: number;
	frozenFocusY: number;
	focusXSpring?: SpringState;
	focusYSpring?: SpringState;
}
...
export function createCursorFollowCameraState(): CursorFollowCameraState {
	return {
		...
		frozenFocusX: 0.5,
		frozenFocusY: 0.5,
		focusXSpring: createSpringState(0.5),
		focusYSpring: createSpringState(0.5),
	};
}
...
export function resetCursorFollowCamera(state: CursorFollowCameraState): void {
	...
	if (state.focusXSpring) resetSpringState(state.focusXSpring, 0.5);
	if (state.focusYSpring) resetSpringState(state.focusYSpring, 0.5);
}
...
export function computeCursorFollowFocus(
	...
	if (!state.initialized || !state.wasZoomed || timeWentBackwards) {
		...
		if (state.focusXSpring) resetSpringState(state.focusXSpring, initialFocus.cx);
		if (state.focusYSpring) resetSpringState(state.focusYSpring, initialFocus.cy);
		return initialFocus;
	}

	const deltaMs = Math.min(80, Math.max(1, timeMs - state.lastTimeMs));
	state.lastTimeMs = timeMs;
	...
	if (config.springDamping && state.focusXSpring && state.focusYSpring) {
		...
		state.frozenFocusY = smoothedY;
		return clampFocusToScale({ cx: smoothedX, cy: smoothedY }, zoomScale);
	}
