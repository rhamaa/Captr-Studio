...
describe("timeline dnd engine", () => {
	...
	it("magnetically snaps time to closest snap point within threshold", () => {
		const snapPoints = [0, 1000, 2500, 5000];
		expect(snapTimeToPoints(950, snapPoints, 100)).toBe(1000);
		expect(snapTimeToPoints(1040, snapPoints, 100)).toBe(1000);
		expect(snapTimeToPoints(1150, snapPoints, 100)).toBe(1150); // outside threshold
	});

	it("magnetically snaps edge during resize", () => {
		const result = resolveResizeEnd(
			"a",
			{ start: 0, end: 1480 },
			{
		...
		expect(result?.end).toBe(1500); // snapped from 1480 to 1500
	});

	it("magnetically snaps position during drag", () => {
		const result = resolveDragEnd("b", { start: 1020, end: 2020 }, "row-clip", {
			allRegionSpans: BASE_SPANS,
			totalMs: 5000,
			minItemDurationMs: 100,
			hasOverlap: () => false,
		...
		expect(result?.span.start).toBe(1000); // snapped from 1020 to 1000
	});
});
