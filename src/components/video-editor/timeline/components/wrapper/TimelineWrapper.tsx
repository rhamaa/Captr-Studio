...
export default function TimelineWrapper({
	...
	const snapPoints = useMemo(() => {
		...
		points.add(0);
		if (totalMs > 0) points.add(totalMs);
		for (const region of allRegionSpans) {
			points.add(region.start);
			points.add(region.end);
		}
		return Array.from(points);
	}, [allRegionSpans, totalMs]);
	...
	const onResizeEnd = useCallback(
		...
		[allRegionSpans, hasOverlap, minItemDurationMs, onItemSpanChange, snapPoints, totalMs],
	);
