...
describe("buildInteractionZoomSuggestions (click-cluster logic)", () => {
	...
	it("adapts zoom depth based on cluster bounding box span", () => {
		...
		expect(wideResult.status).toBe("ok");
		...
		expect(modResult.status).toBe("ok");
		expect(modResult.suggestions[0].depth).toBe(2);
	});

	it("biases framing downward for dropdown-open and centers text-selection", () => {
		const dropdownClick: CursorTelemetryPoint = {
			timeMs: 2_000,
			cx: 0.5,
			cy: 0.2,
			interactionType: "click",
		};
		// Followed by downward move to trigger dropdown-open classification
		const dropdownTelemetry: CursorTelemetryPoint[] = [
			makeMove(0),
			dropdownClick,
			makeMove(2_200, 0.5, 0.25),
			makeMove(2_400, 0.5, 0.35),
			makeMove(2_600, 0.5, 0.45),
			makeMove(TOTAL_MS),
		];

		const result = buildInteractionZoomSuggestions({
			cursorTelemetry: dropdownTelemetry,
			totalMs: TOTAL_MS,
			defaultDurationMs: 3_000,
		});

		expect(result.status).toBe("ok");
		// cy should be biased downward > 0.2
