	...
	useEffect(() => {
		...
		for (const track of resolvedSourceTracks) {
			...
			audio.onerror = () => {
				...
				latestAudio.src = "";
				onSourceFallbackLoadError(
					new Error(`Failed to play companion audio: ${audioPath}`),
				);
			};
	...
	useEffect(() => {
		...
		for (const track of resolvedUserTracks) {
			...
			if (isPlaying && isInRegion) {
				...
				const audioOffset =
					((track.sourceOffsetMs ?? 0) +
						(currentTimeMs - startMs) * (track.playbackRate ?? 1)) /
					1000;
				if (Math.abs(audio.currentTime - audioOffset) > 0.2) {
					audio.currentTime = audioOffset;
				}
				const syncedPlaybackRate = getMediaSyncPlaybackRate({
					basePlaybackRate: targetPlaybackRate * (track.playbackRate ?? 1),
