	...
	return (
		<div className="flex items-center gap-0.5 text-xs text-foreground/80 font-medium select-none">
			...
			<DropdownMenu>
				...
				<DropdownMenuContent align="start" className="min-w-[210px] bg-editor-dialog border border-foreground/15 shadow-xl rounded-xl p-1 text-xs">
					<DropdownMenuItem onClick={onNewProject} className="cursor-pointer gap-2 py-1.5">
						<FilePlusIcon className="w-4 h-4 text-primary" />
						<span>New Project</span>
						<DropdownMenuShortcut>{mod}N</DropdownMenuShortcut>
					</DropdownMenuItem>

					<DropdownMenuItem onClick={onOpenProjectFile} className="cursor-pointer gap-2 py-1.5">
							...
							<DropdownMenuSubContent className="min-w-[240px] max-h-[320px] overflow-y-auto bg-editor-dialog border border-foreground/15 shadow-xl rounded-xl p-1 text-xs">
								{recentProjects.length > 0 ? (
									recentProjects.slice(0, 8).map((proj) => (
										...
										>
											<span className="font-medium text-foreground truncate max-w-[210px]">{proj.name}</span>
											<span className="text-[10px] text-muted-foreground truncate max-w-[210px]">{proj.path}</span>
										</DropdownMenuItem>
									))
								) : (
					...
					<DropdownMenuItem onClick={onSaveProject} className="cursor-pointer gap-2 py-1.5">
						<span>Save Project</span>
						<DropdownMenuShortcut>{mod}S</DropdownMenuShortcut>
					</DropdownMenuItem>

					<DropdownMenuItem onClick={onSaveAsProject} className="cursor-pointer gap-2 py-1.5">
						<span>Save As...</span>
					...
					<DropdownMenuItem onClick={onImportMedia} className="cursor-pointer gap-2 py-1.5">
						<PlusIcon className="w-4 h-4 text-muted-foreground" />
						<span>Import Media...</span>
						<DropdownMenuShortcut>{mod}I</DropdownMenuShortcut>
					</DropdownMenuItem>

					<DropdownMenuItem onClick={onExportVideo} className="cursor-pointer gap-2 py-1.5 font-semibold text-primary">
					...
					<DropdownMenuItem onClick={onNavigateToWelcome} className="cursor-pointer gap-2 py-1.5">
			...
			<DropdownMenu>
				...
				<DropdownMenuContent align="start" className="min-w-[190px] bg-editor-dialog border border-foreground/15 shadow-xl rounded-xl p-1 text-xs">
					<DropdownMenuItem onClick={onUndo} disabled={!canUndo} className="cursor-pointer gap-2 py-1.5">
						<span>Undo</span>
						<DropdownMenuShortcut>{mod}Z</DropdownMenuShortcut>
					</DropdownMenuItem>

					<DropdownMenuItem onClick={onRedo} disabled={!canRedo} className="cursor-pointer gap-2 py-1.5">
						<span>Redo</span>
					...
					{onSplitClip && (
						<DropdownMenuItem onClick={onSplitClip} className="cursor-pointer gap-2 py-1.5">
					...
					{onDeleteClip && (
						<DropdownMenuItem onClick={onDeleteClip} className="cursor-pointer gap-2 py-1.5 text-destructive focus:text-destructive">
					...
					{onSelectAllClips && (
						<DropdownMenuItem onClick={onSelectAllClips} className="cursor-pointer gap-2 py-1.5">
			...
			<DropdownMenu>
				...
				<DropdownMenuContent align="start" className="min-w-[190px] bg-editor-dialog border border-foreground/15 shadow-xl rounded-xl p-1 text-xs">
					{onZoomInTimeline && (
						<DropdownMenuItem onClick={onZoomInTimeline} className="cursor-pointer gap-2 py-1.5">
					...
					{onZoomOutTimeline && (
						<DropdownMenuItem onClick={onZoomOutTimeline} className="cursor-pointer gap-2 py-1.5">
					...
					{onFitTimeline && (
						<DropdownMenuItem onClick={onFitTimeline} className="cursor-pointer gap-2 py-1.5">
					...
					{onToggleSocialSafeZone && (
						<>
							<DropdownMenuSeparator className="bg-foreground/10 my-1" />
							<DropdownMenuItem onClick={onToggleSocialSafeZone} className="cursor-pointer gap-2 py-1.5">
								<span className="flex-1">Social Safe Zones</span>
								{showSocialSafeZone && <CheckIcon className="w-3.5 h-3.5 text-primary" />}
							</DropdownMenuItem>
						</>
					)}
				</DropdownMenuContent>
			...
			<DropdownMenu>
				...
				<DropdownMenuContent align="start" className="min-w-[210px] bg-editor-dialog border border-foreground/15 shadow-xl rounded-xl p-1 text-xs">
					<DropdownMenuItem onClick={() => onOpenSettings("general")} className="cursor-pointer gap-2 py-1.5">
						<GearIcon className="w-4 h-4 text-muted-foreground" />
						<span>Preferences...</span>
						<DropdownMenuShortcut>{mod},</DropdownMenuShortcut>
					</DropdownMenuItem>

					<DropdownMenuItem onClick={onOpenShortcuts} className="cursor-pointer gap-2 py-1.5">
					...
					{onOpenProjectsFolder && (
						<DropdownMenuItem onClick={onOpenProjectsFolder} className="cursor-pointer gap-2 py-1.5">
							<FolderIcon className="w-4 h-4 text-muted-foreground" />
					...
					{onOpenRecordingsFolder && (
						<DropdownMenuItem onClick={onOpenRecordingsFolder} className="cursor-pointer gap-2 py-1.5">
							<FolderOpenIcon className="w-4 h-4 text-muted-foreground" />
			...
			<DropdownMenu>
				...
				<DropdownMenuContent align="start" className="min-w-[190px] bg-editor-dialog border border-foreground/15 shadow-xl rounded-xl p-1 text-xs">
					...
					<DropdownMenuItem onClick={() => onOpenSettings("about")} className="cursor-pointer gap-2 py-1.5">
