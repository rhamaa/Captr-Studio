				...
				<span title={t("recording.micToggleDisabledTip")}>
					<Button
						...
						className={`relative transition-all duration-150 rounded-[11px] ${
							microphoneEnabled
								? "text-emerald-400 bg-emerald-500/15 border border-emerald-500/35 shadow-[0_0_10px_rgba(52,211,153,0.25)] opacity-90"
								: "text-red-400/90 bg-red-500/10 border border-red-500/25 opacity-70"
						}`}
					...
					>
						{microphoneEnabled ? (
							<>
								...
								<span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-emerald-400 ring-2 ring-[#121215] shadow-[0_0_6px_rgba(52,211,153,0.9)]" />
							</>
						) : (
							<>
								<MicrophoneSlashIcon size={18} />
								<span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-red-400/80 ring-2 ring-[#121215]" />
							</>
						)}
					</Button>
				</span>
