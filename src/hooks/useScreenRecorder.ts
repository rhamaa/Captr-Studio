...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const cachedPlatform = useRef<string | null>(null);
	const cachedNativeWindowsCaptureAvailable = useRef<boolean | null>(null);
	...
	/** Pre-acquired mic stream obtained during prepare phase (before native recording starts). */
	const preparedMicStream = useRef<MediaStream | null>(null);
	...
	const preparePermissions = useCallback(async (options: { startup?: boolean } = {}) => {
		...
		if (!screenPermission.success || screenPermission.status !== "granted") {
			...
			alert(
				options.startup
					? "Captr Studio needs Screen Recording permission before you start. System Settings has been opened. After enabling it, quit and reopen Captr Studio."
					: "Screen Recording permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Captr Studio before recording.",
			);
			return false;
		}
		...
		alert(
			options.startup
				? "Captr Studio also needs Accessibility permission for cursor tracking. System Settings has been opened. After enabling it, quit and reopen Captr Studio."
				: "Accessibility permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Captr Studio before recording.",
		);
	...
	const cleanupCapturedMedia = useCallback(() => {
		...
		if (preparedMicStream.current) {
			preparedMicStream.current.getTracks().forEach((track) => track.stop());
	...
	const prepareWebcamRecorder = useCallback(async () => {
		...
		// If webcamStream is already active and live (e.g. pre-warmed), reuse it
		if (
			webcamStream.current &&
			webcamStream.current.active &&
			webcamStream.current.getVideoTracks().some((track) => track.readyState === "live") &&
			webcamRecorder.current &&
			webcamRecorder.current.state === "inactive"
		) {
			return;
		}
	...
	/**
	 * Pre-acquire the microphone stream during the prepare phase so that
	 * when recording starts, the mic is already open and there is no
	 * getUserMedia latency (100-500 ms) in the hot path.
		...
		if (nativeScreenRecording.current) {
			...
			void (async () => {
				...
				try {
					...
					console.log(
						`[PERF:RENDERER] Native Stop Sequence: COMPLETED in ${(performance.now() - stopStart).toFixed(2)}ms`,
					);
				} catch (bgError) {
					console.error("Error in recording finalization:", bgError);
	...
	const startRecording = async () => {
		...
		try {
			...
			if (
				...
				if (cachedNativeWindowsCaptureAvailable.current !== null) {
					useNativeWindowsCapture = cachedNativeWindowsCaptureAvailable.current;
				} else {
					try {
				...
				if (nativeResult.success) {
					...
					if (nativeResult.microphoneFallbackRequired && microphoneEnabled) {
						...
						try {
							...
							// Use pre-warmed mic stream if available to eliminate async getUserMedia latency
							let micStream = preparedMicStream.current;
							preparedMicStream.current = null;
							if (
								!micStream ||
								!micStream.active ||
							...
							void createAudioInputDeviceSnapshot()
								.then((snapshot) => {
									micFallbackAudioInputDevices.current = snapshot;
									console.info(
					...
					} else if (preparedMicStream.current) {
						// Fallback mic was not required; release the pre-warmed stream
						preparedMicStream.current.getTracks().forEach((track) => track.stop());
						preparedMicStream.current = null;
					}
				...
				try {
					...
					if (videoResult.path) {
						...
						try {
							...
							console.error("Error finalizing browser recording session:", bgError);
