...
export class FrameRenderer {
	...
	private applyWebcamLayout(nextLayout: WebcamLayoutCache): void {
		...
		const targetWidth = nextLayout.width ?? nextLayout.size;
		const targetHeight = nextLayout.height ?? nextLayout.size;
		...
		for (const layer of this.webcamShadowLayers) {
			...
			const offsetY = targetHeight * layer.offsetScale * nextLayout.shadowStrength;
	...
	private async syncWebcamFrame(targetTime: number): Promise<void> {
		...
		this.webcamSeekPromise = new Promise<void>((resolve) => {
			...
			const finish = () => {
				...
				settled = true;
				if (Math.abs(webcamVideo.currentTime - clampedTime) <= 0.012) {
					this.lastSyncedWebcamTime = clampedTime;
				}
				cleanup();
				resolve();
	...
	private applyLayoutSceneScreenTransform(layoutCache: LayoutCache): void {
		...
		const webcam = this.config.webcam ?? DEFAULT_WEBCAM_OVERLAY;
	...
	async renderFrame(
		...
		applyClipTransition({
			...
			overlayGraphics: this.transitionOverlayGraphics,
	...
	destroy(): void {
		...
		if (this.transitionOverlayGraphics) {
