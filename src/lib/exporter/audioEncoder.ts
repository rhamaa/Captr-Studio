...
export class AudioProcessor {
	...
	async process(
		...
		// When speed edits, audio regions, ducking, or multiple audio sources need mixing, use offline AudioContext pipeline.
		if (
			...
			Boolean(audioDuckingSettings?.enabled)
		) {
		...
		if (audioDuckingSettings?.enabled) {
			if (mainBufferEntry?.buffer) {
				...
				speechIntervals = getSpeechIntervalsFromChannelData(
					channelData,
					mainBufferEntry.buffer.sampleRate,
				);
			} else if (companionEntries[0]?.buffer) {
				const channelData = companionEntries[0].buffer.getChannelData(0);
				speechIntervals = getSpeechIntervalsFromChannelData(
					channelData,
					companionEntries[0].buffer.sampleRate,
				);
			}
		}
		...
		for (let i = 0; i < chunkCount && !this.cancelled; i++) {
			...
			for (const { buffer, region } of prepared.regionEntries) {
				this.scheduleRegionForChunk(
					...
					outputOffsetSec,
					chunkSec,
					prepared.speechIntervals,
					prepared.audioDuckingSettings,
				);
			}
	...
	private scheduleRegionForChunk(
		...
		const baseGain = Math.max(0, Math.min(1, (region.volume ?? 1) * normalizeGain));
