...
export class FrameRenderer {
	...
	private compositeWithShadows(): void {
		...
		const drawVideoLayer = (targetCtx: CanvasRenderingContext2D) => {
			if (screen.opacity <= 0.001) return;
			targetCtx.save();
			targetCtx.globalAlpha *= screen.opacity;
			if (screen.borderRadius > 0) {
				drawSquircleOnCanvas(targetCtx, {
					x: screen.x,
					y: screen.y,
					width: screen.width,
					height: screen.height,
					radius: screen.borderRadius,
				});
				targetCtx.clip();
			}
			targetCtx.drawImage(videoCanvas, screen.x, screen.y, screen.width, screen.height);
			targetCtx.restore();
		};
	...
	private drawWebcamOverlay(ctx: CanvasRenderingContext2D, width: number, height: number): void {
		...
		const opacity = layoutScene?.webcam.opacity ?? 1;
		if (opacity <= 0.001) {
			return;
		}
		...
		const bubbleHeight = Math.max(1, Math.ceil(webcamHeight));
		...
		const coverScale = Math.max(size / sw, webcamHeight / sh);
		...
		if ((layoutScene?.webcam.shadow ?? webcam.shadow ?? 0) > 0) {
			const shadow = Math.max(0, Math.min(1, layoutScene?.webcam.shadow ?? webcam.shadow));
			ctx.save();
			ctx.globalAlpha *= opacity;
			ctx.filter = `drop-shadow(0 ${Math.round(webcamHeight * 0.06)}px ${Math.round(webcamHeight * 0.22)}px rgba(0,0,0,${shadow}))`;
			ctx.drawImage(bubbleCanvas, x, y, size, webcamHeight);
			ctx.restore();
			return;
		}
