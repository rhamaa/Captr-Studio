...
export function isPathInsideDirectory(candidatePath: string, directoryPath: string) {
	...
	const b =
		process.platform === "win32"
			? normalizedDirectoryPath.toLowerCase()
...
export function isAllowedLocalReadPath(candidatePath: string) {
	...
	const isPathApproved = (targetPath: string) => {
		if (process.platform === "win32") {
			const lowerTarget = targetPath.toLowerCase();
			for (const approved of approvedLocalReadPaths) {
				if (approved.toLowerCase() === lowerTarget) return true;
			}
			return false;
		}
		return approvedLocalReadPaths.has(targetPath);
	};
	...
	const lexicalAllowed =
		...
		isPathApproved(normalizedCandidatePath);
	if (!lexicalAllowed) {
		return false;
	}

	const sameAsNormalized =
		process.platform === "win32"
			? canonicalCandidatePath.toLowerCase() === normalizedCandidatePath.toLowerCase()
			: canonicalCandidatePath === normalizedCandidatePath;
	...
	return (
		...
		isPathApproved(canonicalCandidatePath)
	);
}
