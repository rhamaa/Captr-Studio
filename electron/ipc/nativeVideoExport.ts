...
export function getLibx264ModeArgs(encodingMode: NativeExportEncodingMode): string[] {
	switch (encodingMode) {
		case "fast":
...
export function getNvencVideoModeArgs(encodingMode: NativeExportEncodingMode): string[] {
	...
	switch (encodingMode) {
		case "quality":
			return ["-preset", "p4", "-tune", "hq", ...lowLatencyArgs];
		case "balanced":
			return ["-preset", "p2", "-tune", "ll", ...lowLatencyArgs];
		case "fast":
		default:
			return ["-preset", "p1", "-tune", "ull", ...lowLatencyArgs];
	}
}

export function getQsvVideoModeArgs(encodingMode: NativeExportEncodingMode): string[] {
	switch (encodingMode) {
		case "quality":
			return ["-preset", "medium", "-look_ahead", "0"];
		case "balanced":
			return ["-preset", "faster", "-look_ahead", "0"];
		case "fast":
		default:
			return ["-preset", "veryfast", "-look_ahead", "0"];
	}
}

export function getAmfVideoModeArgs(encodingMode: NativeExportEncodingMode): string[] {
	switch (encodingMode) {
		case "quality":
			return ["-quality", "quality", "-rc", "vbr_latency", "-header_insertion", "1"];
		case "balanced":
			return ["-quality", "balanced", "-rc", "vbr_latency", "-header_insertion", "1"];
		case "fast":
		default:
			return ["-quality", "speed", "-rc", "vbr_latency", "-header_insertion", "1"];
	}
}

export function getVideoToolboxModeArgs(encodingMode: NativeExportEncodingMode): string[] {
	switch (encodingMode) {
		case "quality":
			return ["-realtime", "0", "-allow_sw", "0"];
		case "balanced":
		case "fast":
		default:
			return ["-realtime", "1", "-allow_sw", "0"];
	}
}

export function getMediaFoundationModeArgs(encodingMode: NativeExportEncodingMode): string[] {
	switch (encodingMode) {
		case "quality":
			return ["-ratecontrol", "1", "-scenario", "display"];
		case "balanced":
		case "fast":
		default:
			return ["-ratecontrol", "0", "-scenario", "display"];
	}
}

export function getEncoderModeArgs(
	encoder: string,
	encodingMode: NativeExportEncodingMode,
): string[] {
	switch (encoder) {
		case "h264_nvenc":
			return getNvencVideoModeArgs(encodingMode);
		case "h264_qsv":
			return getQsvVideoModeArgs(encodingMode);
		case "h264_amf":
			return getAmfVideoModeArgs(encodingMode);
		case "h264_videotoolbox":
			return getVideoToolboxModeArgs(encodingMode);
		case "h264_mf":
			return getMediaFoundationModeArgs(encodingMode);
		case "libx264":
			return getLibx264ModeArgs(encodingMode);
		default:
			return [];
	}
}
...
export function buildNativeVideoExportArgs(
	...
	args.push(...getEncoderModeArgs(encoder, options.encodingMode));
...
export function buildNativePrecompositedStaticLayoutArgs(
	...
	const selectedEncoder = config.encoder ?? "h264_nvenc";
	args.push(
		"-filter_complex",
		filterComplex,
		"-map",
		...
		...(selectedEncoder === "h264_nvenc"
			? getNvencStaticLayoutModeArgs(config.encodingMode)
			: getEncoderModeArgs(selectedEncoder, config.encodingMode)),
		...getBitrateArgs(config.bitrate),
		"-pix_fmt",
		"yuv420p",
		"-movflags",
