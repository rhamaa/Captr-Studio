...
let cachedWindowsCaptureAvailable: boolean | null = null;

export async function isNativeWindowsCaptureAvailable(): Promise<boolean> {
	if (cachedWindowsCaptureAvailable !== null) {
		return cachedWindowsCaptureAvailable;
	}
	if (process.platform !== "win32") {
		cachedWindowsCaptureAvailable = false;
		return false;
	}
	...
	try {
		...
		cachedWindowsCaptureAvailable = true;
	} catch {
		cachedWindowsCaptureAvailable = false;
		return false;
	}
