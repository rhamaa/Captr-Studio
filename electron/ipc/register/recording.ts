...
export function registerRecordingHandlers(
		...
		async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
			...
			try {
				...
				if (
					...
					appName &&
					(appName === ownAppName || appName === "captr-studio" || appName === "recordly")
				) {
					return {
						success: false,
						message:
							"Cannot record Captr Studio windows. Please select another app window.",
					};
				}
				...
				if (
					...
					const { response } = await dialog.showMessageBox({
						...
						message:
							"Captr Studio needs screen recording permission to capture your screen.",
						detail: "Please open System Settings > Privacy & Security > Screen Recording, make sure Captr Studio is toggled ON, then try recording again.",
				...
				if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
					const { response } = await dialog.showMessageBox({
						...
						message: "Captr Studio needs microphone permission to record audio.",
						detail: "Please open System Settings > Privacy & Security > Microphone, make sure Captr Studio is toggled ON, then try recording again.",
						buttons: ["Open System Settings", "Cancel"],
						defaultId: 0,
						cancelId: 1,
					});
	...
	ipcMain.handle("list-recordings-files", async () => {
		try {
			const recordingsDir = await getRecordingsDir();
			rememberApprovedLocalReadPath(recordingsDir);
			const entries = await fs.readdir(recordingsDir, { withFileTypes: true });
			const videoExts = new Set([".mp4", ".webm", ".mov", ".mkv", ".m4v"]);
			const files = await Promise.all(
				entries
					.filter((e) => e.isFile() && videoExts.has(path.extname(e.name).toLowerCase()))
					.map(async (e) => {
						const fullPath = path.join(recordingsDir, e.name);
						const stat = await fs.stat(fullPath).catch(() => null);
						return stat
							? {
									name: e.name,
			...
			console.error("Failed to list recordings files:", error);
	...
	ipcMain.handle("stitch-video-clips", async (_, clipPaths: string[]) => {
		if (!Array.isArray(clipPaths) || clipPaths.length === 0) {
			return { success: false, error: "No clips provided" };
		}
		if (clipPaths.length === 1) {
			return { success: true, outputPath: clipPaths[0] };
		}
	...
	ipcMain.handle("record-additional-clip", async () => {
