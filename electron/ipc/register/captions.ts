...
export function registerCaptionHandlers() {
  ...
  ipcMain.handle('get-whisper-model-status', async (_, modelType?: WhisperModelType) => {
    try {
      return await getWhisperModelStatus(modelType || 'small')
    } catch (error) {
      return { success: false, exists: false, path: null, error: String(error) }
    }
  })

  ipcMain.handle('download-whisper-model', async (event, modelType?: WhisperModelType) => {
    const type = modelType || 'small'
    try {
      const existing = await getWhisperModelStatus(type)
      if (existing.exists) {
      ...
      const modelPath = await downloadWhisperModel(event.sender, type)
      return { success: true, path: modelPath }
    } catch (error) {
      console.error(`Failed to download Whisper ${type} model:`, error)
      return { success: false, error: String(error) }
    }
  })

  ipcMain.handle('delete-whisper-model', async (event, modelType?: WhisperModelType) => {
    const type = modelType || 'small'
    try {
      await deleteWhisperModel(type)
      sendWhisperModelDownloadProgress(event.sender, {
        status: 'idle',
        progress: 0,
        path: null,
      ...
      console.error(`Failed to delete Whisper ${type} model:`, error)
      const status = await getWhisperModelStatus(type)
      if (!status.exists) {
        sendWhisperModelDownloadProgress(event.sender, {
          status: 'idle',
          progress: 0,
