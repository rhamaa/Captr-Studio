...
export async function probeAvailableGpuEncoders(ffmpegPath: string): Promise<GpuEncoderInfo> {
	let availableEncoders = new Set<string>();
	try {
		availableEncoders = await getAvailableNativeVideoEncoders(ffmpegPath);
	} catch {
		// ignore
	}

	const hasNvenc = availableEncoders.has("h264_nvenc");
	const hasQsv = availableEncoders.has("h264_qsv");
	const hasAmf = availableEncoders.has("h264_amf");
	const hasVideotoolbox = availableEncoders.has("h264_videotoolbox");
	const hasMf = availableEncoders.has("h264_mf");

	let primaryEncoder = "libx264";
	let primaryEncoderLabel = "CPU Software (x264)";
	let isHardwareAccelerated = false;

	if (hasNvenc) {
		primaryEncoder = "h264_nvenc";
		primaryEncoderLabel = "NVIDIA NVENC";
		isHardwareAccelerated = true;
	} else if (hasQsv) {
		primaryEncoder = "h264_qsv";
		primaryEncoderLabel = "Intel QuickSync";
		isHardwareAccelerated = true;
	} else if (hasAmf) {
		primaryEncoder = "h264_amf";
		primaryEncoderLabel = "AMD VCE / AMF";
		isHardwareAccelerated = true;
	} else if (hasVideotoolbox) {
		primaryEncoder = "h264_videotoolbox";
		primaryEncoderLabel = "Apple VideoToolbox";
		isHardwareAccelerated = true;
	} else if (hasMf) {
		primaryEncoder = "h264_mf";
		primaryEncoderLabel = "Windows Media Foundation";
		isHardwareAccelerated = true;
	}
...
export async function getNativeExportCapabilities(): Promise<NativeExportCapabilities> {
	let gpuEncoders: GpuEncoderInfo | undefined;
	try {
		const ffmpegPath = getFfmpegBinaryPath();
		gpuEncoders = await probeAvailableGpuEncoders(ffmpegPath);
	} catch {
		// ignore
	}
...
export async function exportNativeStaticLayoutVideo(
	...
	try {
		...
		let resolvedEncoder: string | undefined;
		try {
			resolvedEncoder = await resolveNativeVideoEncoder(ffmpegPath, options.encodingMode);
		} catch {
