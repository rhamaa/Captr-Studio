	...
	return {
		...
		detail:
			normalizedProgress >= 100
				? "Finishing the update download. Captr Studio will restart as soon as the installer is ready."
				: remainingMb !== null
					? `${remainingMb.toFixed(1)} MB left before Captr Studio restarts.`
					: "Downloading the update now. Captr Studio will restart when it finishes.",
		delayMs: UPDATE_REMINDER_DELAY_MS,
...
export async function downloadAvailableUpdate(
	...
	setUpdateStatusSummary({
		status: "downloading",
		availableVersion,
		detail: `Downloading Captr Studio ${availableVersion}`,
	});
...
async function showAvailableUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		message: `Captr Studio ${version} is available.`,
...
async function showDownloadedUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		title: "Update Ready",
		message: isPreview
			? `Captr Studio ${version} is ready to install.`
			: `Captr Studio ${version} has been downloaded.`,
		detail: isPreview
...
export function setupAutoUpdates(
	...
	autoUpdater.on("update-available", (info) => {
		...
		setUpdateStatusSummary({
			status: "available",
			availableVersion: info.version,
			detail: `Captr Studio ${info.version} is available.`,
		});
	...
	autoUpdater.on("update-not-available", () => {
		...
		setUpdateStatusSummary({
			status: "up-to-date",
			availableVersion: null,
			detail: `Captr Studio ${app.getVersion()} is up to date.`,
		});
		clearVisibleUpdateToast(sendToRenderer);
		manualCheckRequested = false;
	});
	...
	autoUpdater.on("update-downloaded", (info) => {
		...
		setUpdateStatusSummary({
			status: "ready",
			availableVersion: info.version,
			detail: `Captr Studio ${info.version} is ready to install.`,
		});
		clearDeferredReminderTimer();
