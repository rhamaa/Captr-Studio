...
function createWindow() {
	...
	const createdEditorWindow = createEditorWindowWrapper();
	mainWindow = createdEditorWindow;
...
function focusOrCreateMainWindow() {
	...
	if (!mainWindow || mainWindow.isDestroyed()) {
		const existingEditor = getExistingEditorWindow();
		if (existingEditor && !existingEditor.isDestroyed()) {
			mainWindow = existingEditor;
		} else {
			createWindow();
			return;
		}
...
function getUpdateNotificationTitle(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return `Captr Studio ${payload.version} is available`;
		case "downloading":
			return `Downloading Captr Studio ${payload.version}`;
		case "ready":
			return `Captr Studio ${payload.version} is ready`;
		case "error":
			return `Captr Studio ${payload.version} needs attention`;
	}
}
...
function getUpdateNotificationBody(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return "Click to install the update and restart Captr Studio.";
		case "downloading":
			return "Captr Studio is downloading the update and will restart when it is ready.";
		case "ready":
...
ipcMain.handle("window:set-mode", (event, mode: "welcome" | "editor") => {
	const win = BrowserWindow.fromWebContents(event.sender) ?? mainWindow;
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "Captr Studio";
