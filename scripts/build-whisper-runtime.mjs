...
async function main() {
	...
	for (const target of targets) {
		...
		if (target.platform === "win32") {
			...
			for (const gen of winGenerators) {
				try {
					await rm(target.buildRoot, { recursive: true, force: true });
					await mkdir(target.buildRoot, { recursive: true });
					execFileSync(
						cmake,
						[
			...
			if (!configured) {
				if (softFailAllowed) {
					console.warn(
						"[build-whisper-runtime] Could not configure whisper.cpp with available VS tools. Skipping whisper runtime staging.",
					);
					continue;
				}
				throw new Error(
			...
			try {
				...
				if (softFailAllowed) {
					console.warn(
						`[build-whisper-runtime] Configure failed: ${error.message}. Skipping whisper runtime staging.`,
					);
					continue;
				}
				throw error;
		...
		try {
			...
			if (softFailAllowed) {
				console.warn(
					`[build-whisper-runtime] Build failed: ${buildError.message}. Skipping whisper runtime staging.`,
				);
				continue;
			}
			throw buildError;
